...
#echo "$($PSQL "SELECT (winner.name AS winner_name + opponent.name AS opponent_name) FROM games RIGHT JOIN teams as winner ON games.winner_id=winner.team_id RIGHT JOIN teams as opponent ON games.opponent_id=opponent.team_id WHERE year = 2014 AND round = 'Eighth-Final' ")"
